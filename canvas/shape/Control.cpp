...
CRemoteControl::CRemoteControl(std::istream & input, std::ostream & output, std::shared_ptr<CCanvas> canvas)
    :m_input(input)
    , m_output(output)
    , m_canvas(canvas)
    , m_actionMap({
...
bool CRemoteControl::CreateLine(std::istream & args)
{
    ...
    std::shared_ptr<IShape> shape = std::make_shared<CLineSegment>(startPoint, endPoint, tokens[4]);
